...
async def authorise(
    ...
    try:
        ...
        watchdog_queue.put_nowait(
            f'[{timestamp}] Connection is alive. Prompt before auth'
            )
        ...
        if account_data is None:
            messagebox.showinfo(
                "Invalid token", "Проверьте тоекн. Сервер его не узнал"
                )
        ...
        if event_nickname:
            watchdog_queue.put_nowait(
                f'[{timestamp}] Connection is alive. Authorization done'
                )
...
async def send_msgs(host, port, queue, token, queue_status, watchdog_queue):
    ...
    while True:
        ...
        if msg:
            watchdog_queue.put_nowait(
                f"[{timestamp}] Connection is alive. Message sent"
                )
    ...
    while True:
        try:
            ...
            if cm.expired() is True:
                print("Ошибка, сервер долго не получал пустое сообщение",
                      cm.expired())
                raise gaierror
