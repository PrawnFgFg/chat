        ...
        while True:
            ...
            if queue and not path_history:
                ...
                watchdog_queue.put_nowait(
                    f"[{timestamp}] Connection is alive. New message in chat"
                    )
                logging.info(new_message)
            if path_history:
                async with aiofiles.open(path_history, "a",
                                         encoding='utf-8') as file:
                    await file.write(new_message)
            if print_to_console:
                print(new_message.strip())
    except UnicodeDecodeError:
        logging.info(f"[{now_time}] Попытка декодировать \
            неполные байты как UTF-8 после обрыва соединения")
    finally:
